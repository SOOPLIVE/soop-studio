...
static void shader_filter_reload_effect(struct shader_filter_data *filter) {
    ...
    if (filter->effect == NULL) {
        blog(LOG_WARNING,
             "[obs-shaderfilter] Unable to create effect. Errors returned from "
             "parser:\n%s",
...
static obs_properties_t *shader_filter_properties(void *data) {
    ...
    // TODO: locale.ini 이름 수정 (ExpandLeft -> CropLeft)
    obs_properties_add_int(props, "crop_left", "STR.CropLeft", 0, 9999, 1);
    obs_properties_add_int(props, "crop_right", "STR.CropRight", 0, 9999, 1);
    obs_properties_add_int(props, "crop_top", "STR.CropTop", 0, 9999, 1);
    obs_properties_add_int(props, "crop_bottom", "STR.CropBottom", 0, 9999, 1);
    obs_properties_add_int(props, "repeat_x", "STR.repeat_x", 1, 5, 1);
    obs_properties_add_int(props, "repeat_y", "STR.repeat_y", 1, 5, 1);
    ...
    obs_property_t *from_file = obs_properties_add_bool(
        props, "from_file", obs_module_text("ShaderFilter.LoadFromFile"));
    ...
    for (size_t param_index = 0; param_index < param_count; param_index++) {
        ...
        switch (param->type) {
            ...
            case GS_SHADER_PARAM_FLOAT:
                obs_properties_add_float(props, param_name, param_name,
                                         -FLT_MAX, FLT_MAX, 1);
                break;
            case GS_SHADER_PARAM_INT:
                obs_properties_add_int(props, param_name, param_name, INT_MIN,
                                       INT_MAX, 1);
                break;
            ...
            case GS_SHADER_PARAM_TEXTURE:
                obs_properties_add_path(
                    props, param_name, param_name, OBS_PATH_FILE,
...
static void shader_filter_update(void *data, obs_data_t *settings) {
    ...
    filter->crop_left = (int)obs_data_get_int(settings, "crop_left");
    filter->crop_right = (int)obs_data_get_int(settings, "crop_right");
    filter->crop_top = (int)obs_data_get_int(settings, "crop_top");
    filter->crop_bottom = (int)obs_data_get_int(settings, "crop_bottom");
...
static void shader_filter_tick(void *data, float seconds) {
    ...
    filter->total_width =
        filter->crop_left + base_width + filter->crop_right;
    filter->total_height =
        filter->crop_top + base_height + filter->crop_bottom;

    filter->uv_scale.x = (float)filter->total_width / base_width;
    filter->uv_scale.y = (float)filter->total_height / base_height;

    filter->uv_offset.x = (float)(-filter->crop_left) / base_width;
    filter->uv_offset.y = (float)(-filter->crop_top) / base_height;
