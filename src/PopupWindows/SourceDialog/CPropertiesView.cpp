...
#include "UIComponent/CPlainTextEdit.h"
...
AFQPropertiesView::AFQPropertiesView(OBSData settings_, obs_object_t* obj,
...
AFQPropertiesView::AFQPropertiesView(OBSData settings_, void* obj,
									 PropertiesReloadCallback reloadCallback_,
									 PropertiesUpdateCallback callback_,
									 PropertiesVisualUpdateCb visUpdateCb_,
									 int minSize_)
...
AFQPropertiesView::AFQPropertiesView( OBSData settings_, const char* type_,
									 PropertiesReloadCallback reloadCallback_,
									 int minSize_)
	: VScrollArea(nullptr),
	  properties(nullptr, obs_properties_destroy),
...
AFQPropertiesView::~AFQPropertiesView()
{

}

template<typename Sender, typename SenderParent, typename... Args>
QWidget* AFQPropertiesView::NewWidget(obs_property_t* prop, Sender* widget,
...
QWidget* AFQPropertiesView::AddCheckbox(obs_property_t* prop)
{
...
QWidget* AFQPropertiesView::AddText(obs_property_t* prop, QFormLayout* layout,
	QLabel*& label)
{
	...
	if (type == OBS_TEXT_MULTILINE) {
		AFQPlainTextEdit* edit = new AFQPlainTextEdit(this, monospace);
		edit->setFixedHeight(80);
		edit->setPlainText(QT_UTF8(val));
		edit->setTabStopDistance(40);
		return NewWidget(prop, edit, &AFQPlainTextEdit::textChanged);
	}
		...
		show->setText(localeManager.Str("Show"));
		...
		connect(show, &QAbstractButton::toggled, [=](bool hide) {
			...
			show->setText(hide ? localeManager.Str("Hide") : localeManager.Str("Show"));
			});
		children.emplace_back(info);
		...
		info_label->setStyleSheet("QLabel { color : rgba(255, 255, 255, 60%); }");
...
void AFQPropertiesView::AddPath(obs_property_t* prop, QFormLayout* layout,
	QLabel** label)
{
	...
	QLayout* subLayout = new QHBoxLayout();
	QLineEdit* edit = new QLineEdit();
	QPushButton* button = new QPushButton(localeManager.Str("Browse"));
...
void AFQPropertiesView::AddInt(obs_property_t* prop, QFormLayout* layout,
							   QLabel** label)
{
	...
	if (type == OBS_NUMBER_SLIDER) {
		QSlider* slider = new AFQSliderIgnoreScroll();
...
void AFQPropertiesView::AddFloat(obs_property_t* prop, QFormLayout* layout,
								 QLabel** label)
{
...
QWidget* AFQPropertiesView::AddList(obs_property_t* prop, bool& warning, bool group)
{
	AFLocaleTextManager& localeManager = AFLocaleTextManager::GetSingletonInstance();
	...
	if (m_ComboBoxFixedSize.width() > -1 && 
		m_ComboBoxFixedSize.height() > -1 &&
		group == false)
		combo->setFixedSize(m_ComboBoxFixedSize);
	else if (group)
		combo->setFixedSize(440, 40);
	else
		combo->setFixedSize(460,40);
	...
	if (obs_data_has_autoselect_value(settings, name)) {
		...
		if (id != -1 && id != idx) {
			...
			QString combined = localeManager.Str("Basic.PropertiesWindow.AutoSelectFormat");
...
void AFQPropertiesView::AddFont(obs_property_t* prop, QFormLayout* layout,
	QLabel*& label)
{
	AFLocaleTextManager& localeManager = AFLocaleTextManager::GetSingletonInstance();
	...
	button->setText(localeManager.Str("Basic.PropertiesWindow.SelectFont"));
...
static void UpdateSimpleFPSSelection(AFQFrameRatePropertyWidget* fpsProps,
...
static QWidget*
CreateSimpleFPSValues(AFQFrameRatePropertyWidget* fpsProps, 
					  bool& selected,
					  const media_frames_per_second* current_fps)
{
...
static void UpdateRationalFPSWidgets(AFQFrameRatePropertyWidget* fpsProps,
									 const media_frames_per_second* current_fps)
{
	if (!current_fps || !media_frames_per_second_is_valid(*current_fps)) {
...
static QWidget* CreateRationalFPS(AFQFrameRatePropertyWidget* fpsProps,
								  bool& selected,
								  const media_frames_per_second* current_fps)
{
	auto widget = new QWidget{};
...
static AFQFrameRatePropertyWidget*
CreateFrameRateWidget(obs_property_t* prop, bool& warning, 
					  const char* option,
					  media_frames_per_second* current_fps,
					  frame_rate_ranges_t& fps_ranges)
{
	auto widget = new AFQFrameRatePropertyWidget{};
...
static void UpdateMinMaxLabels(AFQFrameRatePropertyWidget* w)
{
	auto Hide = [&](bool hide) {
...
static void UpdateFPSLabels(AFQFrameRatePropertyWidget* w)
{
...
void AFQPropertiesView::AddFrameRate(obs_property_t* prop, bool& warning,
...
void AFQPropertiesView::AddEditableList(obs_property_t* prop,
	QFormLayout* layout, QLabel*& label)
{
	const char* name = obs_property_name(prop);
...
QWidget* AFQPropertiesView::AddButton(obs_property_t* prop)
{
	const char* desc = obs_property_description(prop);
...
void AFQPropertiesView::AddGroup(obs_property_t* prop, QFormLayout* layout)
{
	const char* name = obs_property_name(prop);
	bool val = obs_data_get_bool(settings, name);
	const char* desc = obs_property_description(prop);
...
void AFQPropertiesView::AddColorInternal(obs_property_t* prop,
	QFormLayout* layout, QLabel*& label,
	bool supportAlpha)
{
	AFLocaleTextManager& localeManager = AFLocaleTextManager::GetSingletonInstance();
	...
	//button->setProperty("themeID", "settingsButtons");
	button->setFixedHeight(40);
	button->setText(localeManager.Str("Basic.PropertiesWindow.SelectColor"));
	button->setToolTip(QT_UTF8(obs_property_long_description(prop)));
...
void AFQPropertiesView::AddColor(obs_property_t* prop, QFormLayout* layout,
	QLabel*& label)
{
	AddColorInternal(prop, layout, label, false);
}

void AFQPropertiesView::AddColorAlpha(obs_property_t* prop, QFormLayout* layout,
	QLabel*& label)
{
	AddColorInternal(prop, layout, label, true);
}

void AFQPropertiesView::AddProperty(obs_property_t* property, QFormLayout* layout, bool group)
{
	...
	if (label) {
		...
		if (m_LabelFixedSize.width() > -1 && m_LabelFixedSize.height() > -1)
			label->setFixedSize(m_LabelFixedSize);
...
void AFQPropertiesView::CreatePropsMainWidget()
{
	...
	if (source) {
		...
		m_bAdvancedProps = config_get_bool(config.GetGlobal(), "PropertiesView-Mode", sourceId);
	}

	m_propWidget = new QWidget();
	m_propWidget->setObjectName("PropertiesContainer");

	m_formLayout = new QFormLayout(m_propWidget);
	...
	m_propWidget->setLayout(m_formLayout);

	QSizePolicy mainPolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);

	setWidgetResizable(true);
	setWidget(m_propWidget);
	setSizePolicy(mainPolicy);
}

void AFQPropertiesView::AddControlPanelButton(QFormLayout* layout)
{
    QIcon icon;
    LoadIconFromABSPath("assets/source-props/ic_source_control.svg", icon);
	...
	controlButton->setText(Str("Basic.SourceProperties.SourceControl"));
	controlButton->setFixedHeight(40);
	controlButton->setIcon(icon);

	connect(controlButton, &QPushButton::clicked, 
			this, &AFQPropertiesView::qSlotShowControlPanel);

	layout->addRow(controlButton);
}

void AFQPropertiesView::AddAdvancedPropButton(QFormLayout* layout)
{
	...
	if (m_bAdvancedProps) {
        LoadIconFromABSPath("assets/source-props/hide_prop.svg", icon);
		advancedButton->setText("Simple ");
	}
	else {
        LoadIconFromABSPath("assets/source-props/show_prop.svg", icon);
	...
	connect(advancedButton, &QPushButton::clicked, this, &AFQPropertiesView::ShowAdvancedProperties);

	layout->addRow(advanceLine);
	layout->addRow(advancedButton);
}

bool AFQPropertiesView::IsHideAdvancedProperties()
{
	OBSSource source = GetSourceObject();
...
void AFQPropertiesView::SetScrollPos(int h, int v, int old_hend, int old_vend)
{
...
void AFQPropertiesView::GetScrollPos(int& h, int& v, int& hend, int& vend)
{
	h = v = 0;
...
inline OBSSource AFQPropertiesView::GetSourceObject()
{
	OBSSource source;

	void* ptr = GetOBSObject().Get();
	source = static_cast<obs_source_t*>(ptr);

	return source;
}

bool AFQPropertiesView::IsExistAdvanceMode(QString id)
{
...
void AFQPropertiesView::SetAdvancedPropertiesById(QString id, bool showAdvancedProps)
{
	if (0 == id.compare("game_capture"))
...
void AFQPropertiesView::SetGameCaptureProps(bool showProp)
{
...
void AFQPropertiesView::SetWindowCaptureProps(bool showProp)
{
	set_visible_prop("method", showProp);
	set_visible_prop("priority", showProp);
}

void AFQPropertiesView::SetDshowInputProps(bool showProp)
{
...
void AFQPropertiesView::SetFFmpegSourceProps(bool showProp)
{
...
void AFQPropertiesView::SetTextGdiPlusProps(bool showProp)
{
...
void AFQPropertiesView::SetBrowserSourceProps(bool showProp)
{
...
void AFQPropertiesView::qslotResizeWidget(bool empty)
{
	if (m_bScrollEnabled)
		return;
	...
	if (m_nPropertiesViewWidth > -1)
		m_propWidget->setFixedWidth(m_nPropertiesViewWidth);

	m_propWidget->adjustSize();

	if (empty)
		setFixedHeight(0);
	else
		setFixedHeight(m_propWidget->height());
}

void AFQPropertiesView::resizeEvent(QResizeEvent* event)
{
	//emit PropertiesResized();
	VScrollArea::resizeEvent(event);
}

void AFQPropertiesView::RefreshProperties()
{
	...
	m_propWidget->setUpdatesEnabled(false);
	m_propWidget->setLayout(nullptr);
	
	children.clear();
	clearLayout(m_formLayout);

	qslotResizeWidget(true);

	if(m_nLayoutMargin[0] > -1 && m_nLayoutMargin[1] > -1 && m_nLayoutMargin[2] > -1 && m_nLayoutMargin[3] > -1)
        m_formLayout->setContentsMargins(m_nLayoutMargin[0], m_nLayoutMargin[1], m_nLayoutMargin[2], m_nLayoutMargin[3]);
	if (m_nFormVSpacing > -1)
        m_formLayout->setVerticalSpacing(m_nFormVSpacing);
	if (m_nFormHSpacing > -1)
        m_formLayout->setHorizontalSpacing(m_nFormHSpacing);
	...
	if (hasNoProperties) {
		QLabel* noPropertiesLabel = new QLabel(locale.Str("Basic.PropertiesWindow.NoProperties"));
		m_formLayout->addWidget(noPropertiesLabel);
	}


	m_propWidget->setLayout(m_formLayout);
	m_propWidget->setUpdatesEnabled(true);
	
	QMetaObject::invokeMethod(this, "qslotResizeWidget", Qt::QueuedConnection, Q_ARG(bool, false));

	emit PropertiesRefreshed();
}


void AFQPropertiesView::ShowAdvancedProperties()
{
	AFConfigManager& config = AFConfigManager::GetSingletonInstance();
	...
	m_bAdvancedProps = !m_bAdvancedProps;

	if (IsExistAdvanceMode(sourceId)) {
		SetAdvancedPropertiesById(sourceId, m_bAdvancedProps);

		config_set_bool(config.GetGlobal(), "PropertiesView-Mode", sourceId, m_bAdvancedProps);
	}

	RefreshProperties();
}

void AFQPropertiesView::qSlotShowControlPanel()
{
	OBSSource source = GetSourceObject();
	const char* sourceId = obs_source_get_id(source);
	if (0 == strcmp(sourceId, "browser_source")) {
		App()->GetMainView()->GetMainWindow()->ShowBrowserInteractionPopup(source);
		return;
	}

	App()->GetMainView()->GetMainWindow()->ShowOrUpdateContextPopup(source);
}

void AFQPropertiesView::SignalChanged()
{
	emit Changed();
}

void AFQPropertiesView::ReloadProperties()
{
	if (weakObj || rawObj) {
...
static bool FrameRateChangedCommon(AFQFrameRatePropertyWidget *w,
                                   obs_data_item_t *&obj,
                                   const media_frames_per_second *valid_fps)
{
    media_frames_per_second fps{};
...
static bool FrameRateChangedRational(AFQFrameRatePropertyWidget *w,
                                     obs_data_item_t *&obj,
                                     const media_frames_per_second *valid_fps)
{
...
static bool FrameRateChanged(QWidget *widget, const char *name,
                 OBSData &settings)
{
    auto w = qobject_cast<AFQFrameRatePropertyWidget *>(widget);
    if (!w)
        return false;
...
void WidgetInfo::TextChanged(const char* setting)
{
	...
	if (type == OBS_TEXT_MULTILINE) {
		AFQPlainTextEdit* edit =
			static_cast<AFQPlainTextEdit*>(widget);
...
bool WidgetInfo::FontChanged(const char* setting)
{
	...
	if (!font_obj) {
		QFont initial;
		font = AFQCustomFontDialog::getFont(
			&success, initial, view,
			QT_UTF8(localeManager.Str("Basic.PropertiesWindow.SelectFont.WindowTitle")),
			options);
	}
	else {
...
void WidgetInfo::ButtonClicked()
{
	...
	if (type == OBS_BUTTON_URL && strcmp(savedUrl, "") != 0) {
		...
		if (url.isValid() && (url.scheme().compare("http") == 0 ||
			...
			QString msg(
				locale.Str("Basic.PropertiesView.UrlButton.Text"));
			msg += "\n\n";
			msg += QString(locale.Str("Basic.PropertiesView.UrlButton.Text.Url"))
				.arg(savedUrl);

			//QMessageBox::StandardButton button = OBSMessageBox::question(
			//	view->window(),
			//	locale.Str("Basic.PropertiesView.UrlButton.OpenUrl"),
...
void WidgetInfo::EditListAdd()
{
	...
	// [copy-obs]remove : text lookup
	action = new QAction(locale.Str("Basic.PropertiesWindow.AddFiles"), this);
	connect(action, &QAction::triggered, this,
		&WidgetInfo::EditListAddFiles);
	popup.addAction(action);

	action = new QAction(locale.Str("Basic.PropertiesWindow.AddDir"), this);
	connect(action, &QAction::triggered, this, &WidgetInfo::EditListAddDir);
	popup.addAction(action);

	if (type == OBS_EDITABLE_LIST_TYPE_FILES_AND_URLS) {
		// [copy-obs]remove : text lookup
		action = new QAction(locale.Str("Basic.PropertiesWindow.AddURL"),
				     this);
		connect(action, &QAction::triggered, this,
...
void WidgetInfo::EditListAddFiles()
{
	...
	// [copy-obs]remove : text lookup
	QString title = locale.Str("Basic.PropertiesWindow.AddEditableListFiles");
	title = title.arg(QT_UTF8(desc));
	
	QStringList files = OpenFiles(widget, title,
...
void WidgetInfo::EditListAddDir()
{
	...
	QString dir = SelectDirectory(App()->GetMainView(), title,
				      QT_UTF8(default_path));
//#ifdef __APPLE__
//	// TODO: Revisit when QTBUG-42661 is fixed
	widget->window()->raise();
...
void WidgetInfo::EditListEdit()
{
	...
	if (type == OBS_EDITABLE_LIST_TYPE_FILES) {
		...
		// [copy-obs]remove : text lookup
		if (pathDir.exists())
			path = SelectDirectory(widget,
					       Str("Browse"), item->text());
		else
			path = OpenFile(widget, Str("Browse"),
