...
#include "ui_setting-hotkey-area.h"
...
static QLabel* makeLabel(QString text)
{
	...
	label->setProperty("labelType", "settingBold");
...
#undef ADD_HOTKEY_FOCUS_TYPE
	...
	ui->scrollArea_Scene->verticalScrollBar()->setProperty("transparent", true);
	ui->scrollArea_Scene->installEventFilter(this);
}
...
bool AFQHotkeySettingAreaWidget::eventFilter(QObject* watched, QEvent* event)
{
	if (watched == ui->scrollArea_Scene){
		if (event->type() == QEvent::Enter) {
			SetScrollBarTransparent(ui->scrollArea_Scene, ui->scrollAreaWidgetContents_Scene, false);
			return false;
		}
		else if(event->type() == QEvent::Leave) {
			SetScrollBarTransparent(ui->scrollArea_Scene, ui->scrollAreaWidgetContents_Scene, true);
			return false;
		}
	}
...
void AFQHotkeySettingAreaWidget::qslotHotkeyChanged()
{
	...
	m_hotkeysDataChanged =
		any_of(begin(m_hotkeyWigets), end(m_hotkeyWigets),
			[](const HOTKEY_WIDGET_PAIR& hotkey) {
				const auto& hw = *hotkey.second;
				return hw.Changed();
			});
	if (m_hotkeysDataChanged)
		emit qsignalHotkeyChanged();
}
...
void AFQHotkeySettingAreaWidget::UpdateFocusBehaviorComboBox()
{
    ...
    int idx = ui->comboBox_HotkeyFocusBehavior->findData(QT_UTF8(hotkeyFocusType));
    if (idx != -1) {
        ui->comboBox_HotkeyFocusBehavior->setCurrentIndex(idx);
    }
}

void AFQHotkeySettingAreaWidget::SetVerticalScrollBarPosition()
{
	if (m_listSectionPosY.count() > 0)
	{
		m_sectionButtonClicked = true;
		auto position = m_listSectionPosY[1].second - 70;
		ui->scrollArea_Hotkeys->verticalScrollBar()->setValue(position);
		m_sectionButtonClicked = false;
	}
}
...
void AFQHotkeySettingAreaWidget::SaveHotkeysSettings()
{
	...
	if (AFSettingUtils::WidgetChanged(ui->comboBox_HotkeyFocusBehavior)) 
	{
		...
		config_set_string(USERCONFIG, "General", "HotkeyFocusType", QT_TO_UTF8(str));
	}
	...
	for (auto& hotkey : m_hotkeyWigets) {
		...
		config_set_string(config, "Hotkeys", hw.m_name.c_str(), json);
	};

    if (anyUpdate) {
		config_save_safe(config, "temp", nullptr);
		OVERLAY_MANAGER.RefreshHotkey();
    }
...
void AFQHotkeySettingAreaWidget::ClearHotkeyValues() {
    for (HOTKEY_WIDGET_PAIR& hotkeyPair : m_hotkeyWigets) {
...
void AFQHotkeySettingAreaWidget::_AdjustHotkeySectionScroll()
{
	...
	m_listSectionPosY.append(HOTKEY_SECTION_INFO(hotkeyAreaPrev, m_areaTrace));
	...
	for (int i = 0; i < rowCount; i++) {
		...
		if (0 != hotkeyAreaPrev.compare(hotkeyArea)) {
			m_areaTrace += 20;
			m_listSectionPosY.append(HOTKEY_SECTION_INFO(hotkeyArea, m_areaTrace));
			m_areaTrace += 50;

			hotkeyAreaPrev = hotkeyArea;
		}
		m_areaTrace += (50 * widget->GetEditCount());
	}
}
