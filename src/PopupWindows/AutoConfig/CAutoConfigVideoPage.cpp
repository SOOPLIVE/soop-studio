...
#include "ui_auto-config-video-page.h"
...
AFQAutoConfigVideoPage::AFQAutoConfigVideoPage(AFQAutoConfigWizard*parent) :
    QWizardPage(parent),
    ui(new Ui::AFQAutoConfigVideoPage)
{
    ui->setupUi(this);

	//setTitle(QT_UTF8("Basic.AutoConfig.VideoPage"));
	//setSubTitle(QT_UTF8("Basic.AutoConfig.VideoPage.SubTitle"));
	...
	//ui->fps->addItem(QT_UTF8(FPS_PREFER_HIGH_FPS),
	//	(int)AutoConfig::FPSType::PreferHighFPS);
	//ui->fps->addItem(QT_UTF8(FPS_PREFER_HIGH_RES),
	//	(int)AutoConfig::FPSType::PreferHighRes);
	//ui->fps->addItem(QT_UTF8(FPS_USE_CURRENT).arg(fpsStr),
...
AFQAutoConfigVideoPage::~AFQAutoConfigVideoPage()
{
    delete ui;
}

int AFQAutoConfigVideoPage::nextId() const
{
	return 0;
}

bool AFQAutoConfigVideoPage::validatePage()
{
	return false;
}
