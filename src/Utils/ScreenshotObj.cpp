...
ScreenShotObj::ScreenShotObj(obs_source_t* source, bool internalSave_) :
	weakSource(OBSGetWeakRef(source))
{
	internalSave = internalSave_;
	obs_add_tick_callback(ScreenshotTick, this);
}
	...
	if (th.joinable()) {
		...
		if (cx && cy) {
				if (!internalSave)
				{
					...
					text = text.arg(sceneName).arg(QT_UTF8(path.c_str()));

					AFQMessageBox::ShowMessage(QDialogButtonBox::Ok, MAINFRAME, QT_UTF8(""), text, false, true);
				}
		}
		...
		//MAINFRAME->OnEvent(OBS_FRONTEND_EVENT_SCREENSHOT_TAKEN);
...
QPixmap ScreenShotObj::GetPixmap()
{
	if (half_bytes.empty())
...
void ScreenShotObj::Save()
{
	...
	const char* adv_path = strcmp(type, "Standard")
		? config_get_string(activeConfig, "AdvOut", "FFFilePath")
		: config_get_string(activeConfig, "AdvOut", "RecFilePath");

	const char* rec_path = strcmp(mode, "Advanced")
		? config_get_string(activeConfig, "SimpleOutput", "FilePath")
		: adv_path;

	bool noSpace = config_get_bool(activeConfig, "SimpleOutput", "FileNameWithoutSpace");
	const char* filenameFormat = config_get_string(activeConfig, "Output", "FilenameFormatting");
	...
	if (internalSave)
	{
		if (CONFIG_CONTEXT.CheckSavvyTempFolder())
		{
			...
			{
				QString downloadFolder = QString::fromUtf8(savvypath);
				QString downloadImage = downloadFolder + "/savvy.png";
				path = downloadImage.toStdString();
			}
		}
	}
	else
